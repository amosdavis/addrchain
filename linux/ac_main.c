...
static int __init ac_module_init(void)
{
    ...
    ret = ac_claims_init(&ac_claims, AC_DEFAULT_LEASE_BLOCKS, 0, NULL);
    if (ret != AC_OK) {
        ret = -ENOMEM;
        goto fail_claims;
    }

    ret = ac_subnet_init(&ac_subnets, 0, 0, NULL);
    if (ret != AC_OK) {
        ret = -ENOMEM;
        goto fail_subnets;
    }

    ret = ac_partition_init(&ac_parts, 0, 0, NULL);
    if (ret != AC_OK) {
        ret = -ENOMEM;
        goto fail_parts;
    }

    ret = ac_vpn_init(&ac_vpns, 0, NULL);
    if (ret != AC_OK) {
        ret = -ENOMEM;
        goto fail_vpns;
    }
